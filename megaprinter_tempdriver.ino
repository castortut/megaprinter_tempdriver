...
void runExtruderPID(double milliseconds) {
  ...
  if (eSetPoint > 1.0) {  
    ...
    if(extruderHeatingMode == extruderHeat_BangBang) {
      ...
      eErrSum += (eErr * milliseconds)/100;

      eErrSum = max(0, eErrSum);
      eErrSum = min(50000, eErrSum);

     
      // PID tulos
      int p_part = eErr*eKp;
      int i_part = eErrSum*eKi;
      i_part = max(eSetPoint/10,i_part);
      //i_part = min(i_part, eSetPoint/4);


      
      eEffort = constrain(p_part + i_part, 0, 80);  
...
void reportToSerial() {
  ...
  Serial.println("ESP \t ERP \t BSP \t Eff \t pser \t iser");
  
  Serial.print(eSetPoint);
  Serial.print("\t");
  Serial.print(eTemp);
  Serial.print("\t");  
  Serial.print(bSetPoint);
  Serial.print("\t");
  Serial.print(eEffort);
  Serial.print("\t");
  Serial.print(p_ser);
  Serial.print("\t");
  Serial.print(i_ser);
